...
if [[ ! "$DRYRUN" =~ "true" ]]; then
    if [[ "$VERIFY_DATA_STORED" =~ "true" ]]; then
        echo "Validating Data Stored at $OUTPUT_FILE_PATH"
        go run verify-data-stored.go --output-config-file=$OUTPUT_CONFIG_FILE --prefix=$OUTPUT_FILE_PATH
        exit $?
    else
        echo "Running Obslytics"
